...
export function downsamplePixels(pixels, options) {
...
export function analyzePixels(pixels, options) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (options.roi && !pointInRoi(normalizeCentroid(bbox, width, height), options.roi)) continue;
...
function floodFill(pixels, visited, startX, startY, background) {
  const { width, height, data } = pixels;
...
function estimateBackgroundLuma({ width, height, data }) {
  const samples = [];
  const push = (x, y) => samples.push(luma(data, y * width + x));
...
function isForeground(data, pixelOffset, background) {
...
function luma(data, pixelOffset) {
  const i = pixelOffset * 4;
  return data[i] * 0.299 + data[i + 1] * 0.587 + data[i + 2] * 0.114;
}

function chroma(data, pixelOffset) {
  const i = pixelOffset * 4;
...
function normalizeCentroid(bbox, frameWidth, frameHeight) {
  return {
...
function summarize(seeds) {
  ...
  const sum = (key) => seeds.reduce((total, seed) => total + seed[key], 0);
  return {
