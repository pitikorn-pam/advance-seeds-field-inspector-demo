import type { ModelMetadata } from "@/lib/models/types";
import type { PixelImage } from "./ClassicalSeedAnalyzerCore";
import { letterbox, type LetterboxResult, YOLO_INPUT_SIZE } from "./yolo";
...
export type PreprocessProfile =
  | typeof PREPROCESS_PROFILE_RAW_RGB
  | typeof PREPROCESS_PROFILE_MORPH_FUSED_V1;

export type PreprocessProfilePreference = PreprocessProfile | typeof PREPROCESS_PROFILE_MODEL;

export interface PrepareYoloInputOptions {
  profile: PreprocessProfile;
  target?: number;
}

export function prepareYoloInput(
  pixels: PixelImage,
  options: PrepareYoloInputOptions,
): LetterboxResult {
...
export function resolvePreprocessProfile(
  preference: PreprocessProfilePreference,
  metadata?: Pick<ModelMetadata, "preprocess"> | null,
): PreprocessProfile {
...
function morphFusedLetterbox(pixels: PixelImage, target: number): LetterboxResult {
...
function luminancePlane(pixels: PixelImage): Uint8Array {
