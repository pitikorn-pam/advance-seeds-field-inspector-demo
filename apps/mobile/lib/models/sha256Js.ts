...
/**
 * Pure-JS SHA-256 fallback used when the native `expo-crypto` module is
 * not present in the current binary (e.g. an older dev client built
 * before we added the dependency). Roughly 10–20× slower than the
 * native digest but keeps install / verification working without a
 * rebuild.
 */
...
export function sha256BytesJs(bytes: Uint8Array): string {
...
export function sha256Base64Js(base64: string): string {
  return sha256BytesJs(toByteArray(base64));
}
