...
- [x] 4.4 Show pending/failed state on Inspection Detail and disable server-only actions until synced. Obviated by 3.2: pending inspections route to a dedicated `/inspections/pending/[queueId]` page (with status pill, retry, discard, auto-redirect on sync). The canonical `/inspections/[id]` only ever shows synced rows, so there's no pending state to badge there. Re-open if a future "edit notes offline → queue an update" flow lands.
