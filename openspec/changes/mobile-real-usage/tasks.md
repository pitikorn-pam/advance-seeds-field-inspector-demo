...
- [x] 20.4 Android low-pressure live profile — while native live YOLO owns the frame processor, request 1280x720 / 15 fps camera delivery and cap Android native inference to 5 fps until faster delegate/model benchmarks prove 30 fps is sustainable.
- [ ] 20.5 Device verification — rebuild/install on Z Flip 7 FE, confirm Android live YOLO uses the 1280x720 / 15 fps low-pressure profile, capture `AdvanceSeedsTFLite` elapsed timings, and check whether ArUco + YOLO feels smooth without `maxImages` / `FrameProcessorBase` errors.
