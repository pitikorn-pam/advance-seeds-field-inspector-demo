...
- [x] 20.4 Android low-pressure live profile — while native live YOLO owns the frame processor, request 1280x720 / 30 fps camera delivery and cap Android native inference to 5 fps until faster delegate/model benchmarks prove 30 fps inference is sustainable.
...
- [ ] 20.7 Async frame-processor verification — rebuild/install on Z Flip 7 FE, confirm Android live YOLO uses the 1280x720 / 30 fps low-pressure profile, capture `AdvanceSeedsTFLite` elapsed timings/delegate selection, and check whether async inference keeps preview smooth without `maxImages` / `FrameProcessorBase` errors.
